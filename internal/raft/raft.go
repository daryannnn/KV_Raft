...
func (s *Server) HandleAppendEntriesRequest(req AppendEntriesRequest, rsp *AppendEntriesResponse) error {
	...
	//update the server's local commitIndex to the min of req.LeaderCommit and our own log length
...
func (s *Server) Apply(commands [][]byte) ([]ApplyResult, error) {
	...
	//block until we receive results from each of the channels we created
...
// When not just one follower but a quorum of followers all have a matching first N messages,
// the leader can advance the cluster's commitIndex.
